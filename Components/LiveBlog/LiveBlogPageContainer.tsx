...
function LiveBlogPageContainer() {
  ...
  const GetAllListiclesList = async () => {
    setLoader(true);
    const body = {
      n_page: page + 1,
      n_limit: rowsPerPage,
    ...
    const results = await getAllLiveBlogListApi(body);
  ...
  const statusUpdate =async(id: any  , val: any ) =>{
    ...
    if (results?.appStatusCode !== 0) {
      ...
      setLoader(false);
      if(val === 1){
        toast.success("This Live Blog is Active!");
      }else{
        toast.error("This Live Blog is In-Active!");
      }
  ...
  const handleSwitchChange = (e: any, index: number, row: any) => {
    ...
    const  filteredUsers :any = adList.filter((cate :any) => cate.c_listicles_id === row.c_listicles_id);
  ...
  const handleDeleteListicles = async (id: string) => {
    setDeletePop(false);
    const results = await deleteLiveBlogApi(id);
    if (results?.appStatusCode !== 0) {
      toast?.error(results?.error);
    } else {
  ...
  const handleCheck = (e: any, index: any, row: any) => {
    const { checked } = e.target;

    const filteredUsers: any = adList.filter(
      (cate: any) => cate.c_live_blog_id === row.c_live_blog_id
    );
    let valStatus: any = checked ? 1 : 0;

    let arr: any = [...adList];
    checked ? (arr[index] = { ...arr[index], live_status: 1 }): (arr[index] = { ...arr[index], live_status: 0 });
    setAdList(arr);
  ...
  return (
    <Container fluid>
        ...
        <Col sm="12">
          <Card>
            <Box className="d-flex justify-content-between align-items-center">
              ...
              <Box pr={2}>
                <Link href={`/${i18LangStatus}/live-blog/create-live-blog`}>
                  <Button
                    sx={{
                      bgcolor: "#fe6a49",
                      color: "#fff",
              ...
              <Table responsive className={`table-wrapper`}>
                <thead style={{ background: "#000" }}>
                  <tr>
                    ...
                    <th style={{ color: "#fff" }}>Scroll News</th>
                        ...
                        <tr key={index}>
                          ...
                          <td align="left">
                            ...
                            >
                              ...
                              <>
                                <Link
                                  href={`/${i18LangStatus}/live-blog/edit-blog/${row.c_live_blog_id}`}
                                >
      ...
      <DeletePop
        open={deletePop}
        handleDeleteItem={() => handleDeleteListicles(deleteValue)}
        close={closePop}
      />
    </Container>
  );
...
export default LiveBlogPageContainer;
