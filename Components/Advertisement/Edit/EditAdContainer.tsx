...
// import StyledDemo from "@/CommonComponent/ImageCrop/index";
...
const EditAdContainer = () => {
  ...
  const DeleteImage = async () => {
    ...
    if (results?.appStatusCode !== 0) {
      ...
      dispatch(setCroppedImageSource(""));
      setImageFlag(false);
      toast.success(results?.message);
  ...
  const GetEditAdData = async () => {
    const results = await editAdvertisementApi(
    ...
    if (results?.appStatusCode !== 0) {
      ...
      setInputs({
        ...inputs,
        title: results?.payloadJson?.c_advt_title,
        type: results?.payloadJson?.c_advt_type,
        position: results?.payloadJson?.c_banner_position,
        page: results?.payloadJson?.c_banner_view_pages,
        start_date: dayjs(results?.payloadJson?.c_banner_start_date),
        start_time: dayjs(results?.payloadJson?.c_banner_start_time),
        end_date: dayjs(results?.payloadJson?.c_banner_end_date),
        end_time: dayjs(results?.payloadJson?.c_banner_end_time),
        country: results?.payloadJson?.c_banner_target_country_id,
        state: results?.payloadJson?.c_banner_target_state_id,
        city: results?.payloadJson?.c_banner_target_city_id,
        redirect_url: results?.payloadJson?.c_advt_banner_redirect_url,
        height: results?.payloadJson?.c_banner_height,
        width: results?.payloadJson?.c_banner_width,
        device: results?.payloadJson?.c_target_device,
      });
      setImage({
        ...image,
        name: results?.payloadJson?.c_advt_banner_url?.split("/")?.at(-1),
        src: results?.payloadJson?.c_advt_banner_url,
      });
      setEditFlag(true);
    }
  };
  ...
  const GetcategoriesList = async () => {
    ...
    if (results?.appStatusCode !== 0) {
      ...
      const sortedArray = results?.payloadJson?.sort((a: any, b: any) => {
        if (a._id === inputs?.page) return -1;
        if (b._id === inputs?.page) return 1;
        return 0;
      });

      setInputs({
        ...inputs,
        page: sortedArray[0]?.c_category_id,
      });
