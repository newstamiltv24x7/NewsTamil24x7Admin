      ...
      <Table responsive className={`table-wrapper`}>
        <thead style={{ background: "#000" }}>
          <tr>
            ...
            <th style={{ color: "#fff" }}>Permissions</th>
            ...
            {Array.isArray(adList) &&
              adList?.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>
                    {row?.c_role_privileges?.map((item: any, index: number) => (
                      // <li key={index}>{item?.role_privileage}</li>
                      <Chip key={index} label={`${item?.role_privileage}`} />
                    ))}
                  </td>
