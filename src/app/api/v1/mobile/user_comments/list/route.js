...
function createComments(comments, c_parent_comments_id = null) {
    ...
    if (c_parent_comments_id == null) {
      category = comments.filter((cate) => cate.c_parent_comments_id == undefined && cate.c_comment_id.length > 0);
    } else {
      category = comments.filter((cate) => cate.c_parent_comments_id == c_parent_comments_id && cate.c_comment_id.length > 0);
    }
...
export async function POST(request) {
  ...
  try {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $and: [{ c_user_comment: { $regex: searchTerm, $options: "i" } },{ n_status: 1 }, { n_published: 1 }],
          },
        ];
      } else {
        _search["$and"] = [
        ...
        try {
          ...
          await Comment.aggregate([
            ...
            .then((data) => {
                ...
                const encryptRes = encryptCryptoResponse(comentsData);
                const decryptRes = decrypCryptoRequest(encryptRes);
              if (data.length > 0) {
                sendResponse["appStatusCode"] = 0;
                sendResponse["message"] = "";
