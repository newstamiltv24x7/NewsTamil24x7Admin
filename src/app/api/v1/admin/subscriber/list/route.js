  ...
  try {
    ...
    if (verified.success) {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $or: [{ c_subscriber_email : { $regex: searchTerm, $options: "i" } }],
          },
        ];
      }else {
        _search["$and"] = [
      ...
      if (n_limitTerm !== "" && n_pageTerm !== "") {
        ...
        await Subscriber.aggregate([
          { $match: _search },
          {
            $group: {
              _id: "$_id",
              c_subscriber_email : { $first: "$c_subscriber_email" },
              c_subscriber_id: { $first: "$c_subscriber_id" },
              c_subscriber_activate: { $first: "$c_subscriber_activate" },
...
export async function GET(request) {
  ...
  if (verified.success) {
    if (id) {
      const checkId = await Subscriber.findOne({ c_subscriber_id: id });
      if (checkId) {
        let _search = {};
        _search["$and"] = [
          {
            $and: [{ n_status: 1 }, { n_published: 1 }, { c_subscriber_id: id }],
          },
        ];
      ...
      _search["$and"] = [
        {
          $and: [{ n_status: 1 }, { n_published: 1 }, {c_subscriber_activate: 1}],
        },
      ];
