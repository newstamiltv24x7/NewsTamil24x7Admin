  ...
  try {
    ...
    if (verified.success) {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $or: [{ c_social_handle_category_title: { $regex: searchTerm, $options: "i" } }],
          },
          {
      ...
      if (n_limitTerm !== "" && n_pageTerm !== "") {
        ...
        await SocialHandleCategory.aggregate([
          { $match: _search },
          {
            $group: {
              _id: "$_id",
              c_social_handle_category_title: { $first: "$c_social_handle_category_title" },
              c_social_handle_category_id: { $first: "$c_social_handle_category_id" },
          ...
          },
          {
            $project: {
              _id: 1,
              c_social_handle_category_title: 1,
...
export async function GET(request) {
  ...
  if (verified.success) {
    if (id) {
      const checkId = await SocialHandleCategory.findOne({ c_social_handle_category_id: id });
      if (checkId) {
        let _search = {};
        _search["$and"] = [
          {
            $and: [{ n_status: 1 }, { n_published: 1 }, { c_social_handle_category_id: id }],
          },
        ];
