    ...
    if (verified.success) {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $or: [
                { c_display_name: { $regex: searchTerm, $options: "i" } },
                { c_from_name: { $regex: searchTerm, $options: "i" } }
            ],
          },
        ];
      }else {
      ...
      if (n_limitTerm !== "" && n_pageTerm !== "") {
        ...
        await Form.aggregate([
          { $match: _search },
          {
            $group: {
              _id: "$_id",
              c_form_id: { $first: "$c_form_id" },
              c_display_name: { $first: "$c_display_name" },
              c_from_name: { $first: "$c_from_name" },
...
export async function GET(request) {
  ...
  if (verified.success) {
    if (id) {
      const checkId = await Form.findOne({ c_form_id: id });
      if (checkId) {
        let _search = {};
        _search["$and"] = [
          {
            $and: [{ n_status: 1 }, { n_published: 1 }, { c_form_id: id }],
          },
        ];
