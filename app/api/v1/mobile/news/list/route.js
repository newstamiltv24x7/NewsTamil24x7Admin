...
function visitCountTotal(visitArray) {
  let returArray = 0;
  visitArray.map((data) => {
    data.map((list) => {
      returArray += list.c_visit_count;
...
function overAllArrayCountTotal(visitArray) {
  let returArray = 0;
  visitArray.map((data) => {
    returArray += data.length;
  });
  return returArray;
}

function overAllCountTotal(visitArray) {
  let returArray = 0;
  returArray += visitArray?.length;
  return returArray;
}
  ...
  data.map((list) => {
    datas.push({
      ...
      visitcount: visitArrayCountTotal(list?.visitcount)
        ...
        : 0,
      over_all_count: overAllArrayCountTotal(list?.visitcount)
        ? overAllArrayCountTotal(list?.visitcount)
        : 0,
...
const seprateData = (data) => {
  ...
  datas.push({
    ...
    visitcount: visitCountTotal(data[0]?.visitcount)
      ? visitCountTotal(data[0]?.visitcount)
      : 0,
    over_all_count: overAllCountTotal(data[0]?.visitcount)
      ? overAllCountTotal(data[0]?.visitcount)
      : 0,
  });
