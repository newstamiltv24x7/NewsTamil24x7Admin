  ...
  try {
    ...
    if (verified.success) {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $or: [{ c_tag_name: { $regex: searchTerm, $options: "i" } }],
          },
        ];
      }else {
        _search["$and"] = [
      ...
      if (n_limitTerm !== "" && n_pageTerm !== "") {
        ...
        await Tags.aggregate([
          { $match: _search },
          {
            $group: {
              _id: "$_id",
              c_tag_name: { $first: "$c_tag_name" },
              c_tag_state: { $first: "$c_tag_state" },
              c_redirect_url: { $first: "$c_redirect_url" },
...
export async function GET(request) {
  ...
  if (verified.success) {
    if (id) {
      const checkId = await Tags.findOne({ c_tag_state: id });
      if (checkId) {
        let _search = {};
        _search["$and"] = [
          {
            $and: [{ n_status: 1 }, { n_published: 1 }, { c_tag_state: id }],
          },
        ];
