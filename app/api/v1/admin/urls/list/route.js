...
export async function POST(request) {
  const { n_page, n_limit, c_search_term, c_video_type,c_youtube_type } = await request.json();

  const verified = verifyAccessToken();
  try {
   
    if (verified.success) {
      if(c_video_type && c_youtube_type){
        let _search = {};
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $and: [ { n_published: 1 },{c_video_type:c_video_type},{c_youtube_type:c_youtube_type},{ c_url_title: { $regex: searchTerm, $options: "i" } }],
          },
        ];
      } else {
        _search["$and"] = [
          {
            $and: [ { n_published: 1 },{c_video_type:c_video_type},{c_youtube_type:c_youtube_type}],
          },
        ];
      }
