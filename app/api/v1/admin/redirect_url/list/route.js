    ...
    if (verified.success) {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $or: [
              { c_redirect_from_url: { $regex: searchTerm, $options: "i" } },
              { c_redirect_to_url: { $regex: searchTerm, $options: "i" } },
            ],
          },
          {
      ...
      if (n_limitTerm !== "" && n_pageTerm !== "") {
        ...
        await RedirectUrl.aggregate([
          { $match: _search },
          {
            $group: {
              _id: "$_id",
              c_redirect_from_url: { $first: "$c_redirect_from_url" },
              c_redirect_url_id: { $first: "$c_redirect_url_id" },
              c_redirect_to_url: { $first: "$c_redirect_to_url" },
...
export async function GET(request) {
  ...
  if (verified.success) {
    if (id) {
      const checkId = await RedirectUrl.findOne({ c_redirect_url_id: id });
      if (checkId) {
        let _search = {};
        _search["$and"] = [
          {
