  ...
  try {
    ...
    if (verified.success) {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $or: [{ c_seo_category_title: { $regex: searchTerm, $options: "i" } }],
          },
          {
      ...
      if (n_limitTerm !== "" && n_pageTerm !== "") {
        ...
        await SeoCategory.aggregate([
          { $match: _search },
          {
            $group: {
              _id: "$_id",
              c_seo_category_title: { $first: "$c_seo_category_title" },
...
export async function GET(request) {
  ...
  if (verified.success) {
    if (id) {
      const checkId = await SeoCategory.findOne({ c_seo_category_id: id });
      if (checkId) {
        let _search = {};
        _search["$and"] = [
          {
            $and: [{ n_status: 1 }, { n_published: 1 }, { c_seo_category_id: id }],
          },
        ];
