    ...
    try {
      ...
      if(sub_id){
        ...
        await Photos.updateOne(
          {"c_photos_continue_item._id" :  sub_id},
          {$set: {
              'c_photos_continue_item.$[].n_status': 0,
              'c_photos_continue_item.$[].n_published': 0
          }},
        ...
        await Photos.findByIdAndUpdate(id,body).then((result) => {
          if (result) {
            sendResponse["appStatusCode"] = 0;
            sendResponse["message"] = "Deleted Successfully";
            sendResponse["payloadJson"] = [];
