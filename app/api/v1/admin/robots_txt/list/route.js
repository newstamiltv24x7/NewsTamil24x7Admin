  ...
  try {
    ...
    if (verified.success) {
      ...
      if (searchTerm !== "") {
        _search["$and"] = [
          {
            $or: [{ c_robots_txt: { $regex: searchTerm, $options: "i" } }],
          },
          {
      ...
      if (n_limitTerm !== "" && n_pageTerm !== "") {
        ...
        await RobotsTxt.aggregate([
          { $match: _search },
          {
            $group: {
              _id: "$_id",
              c_robots_txt: { $first: "$c_robots_txt" },
              c_robots_txt_id: { $first: "$c_robots_txt_id" },
              c_robots_txt_type: { $first: "$c_robots_txt_type" },
...
export async function GET(request) {
  ...
  if (verified.success) {
    if (id) {
      const checkId = await RobotsTxt.findOne({ c_robots_txt_id: id });
      if (checkId) {
        let _search = {};
        _search["$and"] = [
          {
            $and: [{ n_status: 1 }, { n_published: 1 }, { c_robots_txt_id: id }],
          },
        ];
